...
for container in ${containers}; do
  ...
  if [[ -n "$POSTGRES_DB" && -n "$POSTGRES_USER" && -n "$POSTGRES_PASSWORD" ]]; then
    docker exec $container /bin/bash -c "PGPASSWORD=${POSTGRES_PASSWORD} /usr/bin/pg_dump -U ${POSTGRES_USER} ${POSTGRES_DB}" | gzip -9 > "${BACKUP_DIR}/${POSTGRES_DB}_backup.sql.${today}.gz"
  else
